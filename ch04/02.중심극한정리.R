...
# 그래프로 확인
hist(r1.mean, prob=T, xlab='표본평균', ylab='밀도', main='N(2, 1^2)',
...
#표준오차를 줄이고싶으면
#표본의크기를 늘리면 됨 N을 4배를 늘리면 1/2로 줄어든다
...
# 그래프로 확인
par(mfrow=c(2,2)) # 2행2열로 그려라
hist(b2.mean, prob=T, xlab='', ylab='', xlim=c(0,4), 
     main='표본크기: 2', col='orange', border='red')
x1 = seq(min(b2.mean), max(b2.mean), length=1000)
y1 = dnorm(x1, mean=1, sd=sqrt(0.9)/sqrt(2))
lines(x1, y1, lty=2, lwd=2, col='blue')

hist(b4.mean, prob=T, xlab='', ylab='', xlim=c(0,4), 
     main='표본크기: 4', col='orange', border='red')
x2 = seq(min(b4.mean), max(b4.mean), length=1000)
y2 = dnorm(x2, mean=1, sd=sqrt(0.9)/sqrt(4))
lines(x2, y2, lty=2, lwd=2, col='blue')

hist(b32.mean, prob=T, xlab='', ylab='', xlim=c(0,4), 
     main='표본크기: 32', col='orange', border='red')
x3 = seq(min(b32.mean), max(b32.mean), length=1000)
y3 = dnorm(x3, mean=1, sd=sqrt(0.9)/sqrt(32))
lines(x3, y3, lty=2, lwd=2, col='blue')

hist(b64.mean, prob=T, xlab='', ylab='', xlim=c(0,4), 
     main='표본크기: 64', col='orange', border='red')
